...
class CubanWeather(object):
    ...
    def _parse_weather_site_xml(self, xml_content):
        ...
        xml_descriptions = [
            ...
            tree_root.findall('channel/item/description')[1:-1]]
    ...
    def _transform_forecast_data(self):
        ...
        # Convert forecast_data.values() into a list for easily store it in
        # self.forecast_data
        data = []
        for value in self.xml_forecast_data.values():
            data += value
        ...
        for value in self.xml_forecast_data.values():
            for i, datum in enumerate(value):
                if i in range(1, len(value), 5):
                    days.append(datum)
            months += self._test_month_year(days=days, test='month_int')
            years += self._test_month_year(days=days, test='year')
            ...
            for datum in data_to_show:
                if i > 0 and not (i + 1) % 6:
                    forecast_dialog.data_labels[i].source = datum
                      ...
                      and self.prognos_app.temp_unit == u'Fahrenheit ºF'):
